...
impl GhostWriter {
    ...
    /// Generate longevity-aware context with CRITICAL INSTRUCTION block.
    /// This is the enhanced Ghost Writer output that solves the capture problem.
    #[cfg(feature = "longevity")]
    pub fn format_longevity_context(
        longevity_context: &super::longevity::sync::SessionContext,
        v3_context: &SessionResumeResult,
    ) -> String {
        let mut md = String::new();

        // Start with the longevity context (includes CRITICAL INSTRUCTION)
        md.push_str(&longevity_context.to_ghost_writer_format());

        // Append V3 session data
        if !v3_context.decisions.is_empty() {
            md.push_str("## Active Decisions\n");
            for (i, d) in v3_context.decisions.iter().enumerate() {
                md.push_str(&format!("{}. {}\n", i + 1, d));
            }
            md.push('\n');
        }

        if !v3_context.files_touched.is_empty() {
            md.push_str("## Files Modified\n");
            for (path, op) in v3_context.files_touched.iter().take(15) {
                md.push_str(&format!("- `{}` ({})\n", path, op));
            }
            md.push('\n');
        }

        if !v3_context.errors_resolved.is_empty() {
            md.push_str("## Errors Resolved\n");
            for (err, res) in v3_context.errors_resolved.iter().take(5) {
                md.push_str(&format!("- **{}** -> {}\n", err, res));
            }
            md.push('\n');
        }
        ...
        md.push_str("---\n");
        md.push_str("_Auto-generated by AgenticMemory V4 Longevity Engine_\n");
        md
    }
    ...
    fn merge_into_memory_md(memory_file: &Path, context: &SessionResumeResult) {
        ...
        let final_content = edge_cases::merge_preserving_user_sections(&existing, &new_content);
...
mod tests {
    ...
    fn sample_context() -> SessionResumeResult {
        SessionResumeResult {
            ...
            errors_resolved: vec![("missing dep".to_string(), "added to Cargo.toml".to_string())],
