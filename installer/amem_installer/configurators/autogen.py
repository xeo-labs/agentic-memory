"""AutoGen configurator — prints integration instructions."""
...
class AutoGenConfigurator(Configurator):
    """Prints AutoGen integration instructions (no config modification)."""
    ...
    def configure(
        ...
        instructions = (
            f"To integrate AgenticMemory with your AutoGen project, add:\n\n"
            f"    from agentic_memory import Brain\n"
            f'    brain = Brain("{brain_path}")\n\n'
            f"    # Use brain for persistent memory across AutoGen agent sessions"
        )
        return ConfigReport(
